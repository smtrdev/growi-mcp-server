    ...
    switch (name) {
      ...
      case 'mcp_growi_growi_search_pages':
        try {
          ...
          const limit = args.limit !== undefined ? parseInt(args.limit, 10) : 20;
          const offset = args.offset !== undefined ? parseInt(args.offset, 10) : 0;

          logger.info(`Preparing to search pages with query='${query}', limit=${limit}, offset=${offset}`);
          result = await searchPages(growiClient, { query, limit, offset });
          logger.info(`Tool execution completed successfully for '${name}'`);
          return result;
        } catch (error) {
          logger.error(`Error executing '${name}': ${error instanceof Error ? error.message : String(error)}`);
          return { isError: true, content: [{ type: 'text', text: `Error executing tool: ${error instanceof Error ? error.message : String(error)}` }] };
        }
